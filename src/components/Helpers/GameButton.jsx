...
const GameButton = (props) => {
    ...
    const contextMenuOff = (e) => {
        ...
        const flagClick = () => {
            let updatedSquares = squares.map((s) => {
            if (s.id === square.id && !square.isFlagged) {
            ...
            } else if (s.id === square.id && square.isFlagged) {
                props.setFlags((prevFlags) => prevFlags + 1);
                if (square.isMined) {
                props.setMines((prevMines) => prevMines + 1);
                gameContext.setGameSettings((prevSettings) => ({
    ...
    return (
        <button 
            ...
            disabled={square.isRevealed || gameContext.gameSettings.winner === false || square.isFlagged} 
